...
rcube_webmail.prototype.aliases_select = function(list) {
	...
	rcmail.enable_command('plugin.aliases.delete', 'plugin.aliases.enable', true);
	}
...
rcube_webmail.prototype.aliases_update_list = function(action, param1, param2, param3, param4) {
	...
	switch(action) {
		...
		case 'add':
			...
			if (param3) newrow.className = param3;
			if (param1 == -1) {
			...
			rcmail.enable_command('plugin.aliases.delete', 'plugin.aliases.enable', false)
			break;
		case 'update':
	     ...
	     if (rcmail.env.action == 'plugin.aliases') {
		     ...
		     rcmail.register_command('plugin.aliases.delete', function(id) {
			     if (confirm(rcmail.gettext('aliasesaliasdeleteconfirm','aliases'))) {
				     var add_url = '';
				     var target = window;
				     var id=rcmail.aliases_list.get_single_selection()
				     if (rcmail.env.contentframe && window.frames && window.frames[rcmail.env.contentframe]) {
					     add_url = '&_framed=1';
					     target = window.frames[rcmail.env.contentframe];
					     rcube_find_object(rcmail.env.contentframe).style.visibility = 'inherit';
				     }
				     target.location.href = rcmail.env.comm_path+'&_action=plugin.aliases.delete&_iid='+id;
			     }
		     }, false);

		     rcmail.register_command('plugin.aliases.enable', function(id) {
			     var add_url = '';
			     var target = window;
			     var id=rcmail.aliases_list.get_single_selection()
			     if (rcmail.env.contentframe && window.frames && window.frames[rcmail.env.contentframe]) {
			     ...
			     target.location.href = rcmail.env.comm_path+'&_action=plugin.aliases.enable&_iid='+id+'&_framed=1';
		     }, false);
	     }
